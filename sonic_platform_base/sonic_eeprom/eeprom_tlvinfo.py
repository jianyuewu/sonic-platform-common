...
from . import eeprom_tlv_constants
...
class TlvInfoDecoder(eeprom_base.EepromDecoder):
    ...
    def __init__(self, path, start, status, ro, max_len=eeprom_tlv_constants._TLV_INFO_MAX_LEN):
...
# Dynamically add all TLV constants as class attributes for backward compatibility
# This allows subclasses to access constants via self._TLV_CODE_* pattern
for _attr_name in dir(eeprom_tlv_constants):
    if _attr_name.startswith('_TLV_'):
        setattr(TlvInfoDecoder, _attr_name, getattr(eeprom_tlv_constants, _attr_name))
